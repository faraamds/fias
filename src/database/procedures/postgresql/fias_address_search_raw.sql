...
BEGIN
    ...
    from (select *,
          ...
          where CASE WHEN in_regioncode IS NOT NULL THEN fias_address_object.regioncode::INT=in_regioncode ELSE 1=1 END
            ...
            AND to_tsvector('russian', fias_address_object.formalname) @@ q) s
    where s.rank = 1
    order by regionweight desc, weight desc;

    FOR var_address_object IN SELECT * FROM tmp_fias_address_object WHERE aoguid NOT IN (SELECT parentguid FROM tmp_fias_address_object WHERE parentguid IS NOT NULL) order by regionweight desc, weight desc
        LOOP
